...
+ [`shortcutFoo`](https://www.shortcutfoo.com/) teachs you shortcuts for `Vim`, `Emacs`, `Command Line`, `Regex` *etc.* via **interval training**, which is essentially spaced repetition. This helps you memorize shortcuts more efficiently.
...
I had never made an `HTTP` request before and although the process is officially documented [here]((https://dev.fitbit.com/build/reference/web-api/oauth2/)) it can be a *tad* overwhelming. Therefore, I initally resorted to using an `R` package built to access the R API, under-the-hood, called `fitbitr`. 
...
I'm going to grab a week's worth of data for a very prelimary EDA.
...
Since this is a post about `gamification` I decided to do something fun with my exploratory data visualizations. I wanted to use the `Rokemon` package which allows me to set the theme of `ggplot2` (and `ggplot2` extensions) to `Game Boy` and `Game Boy Advance` themes! When convient, I've combined plots with `cowplot`.
...
`Heart rate` is a little right-skewed, probably due to sleep and sedentary work. Similarily, for `step count` you see that only a small bump under `brisk walking pace` from when I skateboarded to work. 
...
It looks like Monday was the day I got my heart rate up the most, the biomodal peak is probably when I was running around looking for a rental property.
...
I'm not sure what that exactly means but apparently the maxium possible MEPS in a 42-minute workout is 168 and since I ran this 10Km in 54:35 I guess that's good?
...
A pie chart is probably not the best way to show this data. Let's visualize the distribution with a boxplot.
...
I would never have considered myself a [Darwin](https://www.psychologytoday.com/us/blog/how-think-about-exercise/201501/charles-darwins-daily-walks) or a [Thoreau](https://en.wikipedia.org/wiki/Walking_(Thoreau)) but apperently *strolling very slowly* is my favorite activity in terms of time spent. 
...
Previously I had always used the `corrplot` package to create a correlation plot; however, it doesn't play nicely with `ggplot` meaning you cannot add `Game Boy` themes easily. Nonetheless, I was able to give it a retro-looking palette with some minor tweeking.

Since I had two colors in mind from the original `gameboy`, and knew their `hex` code, I was able to [generate a pallete from this website](https://meyerweb.com/eric/tools/color-blend/#CADCA0:0F380E:10:hex).
...
In a correlation plot the color of each circle indicates the magnitude of the corrleation, and the size of the circle indicates its significance.
...
Even though Fitbit offers a nice dashboard for a single user it's not scaleable. By accessing the data direcetly one can ask the questions they want from 200 individuals - or more. If one was inclined, they could even build a fancy `Shiny` dashboard with bespoke vizualizations.
